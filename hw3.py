# Libaries required
# !pip install yfinance pandas numpy torch torchmetrics scikit-learn pytorch-lightning tqdm optuna mamba-ssm causal-conv1d>=1.1.0 -q
...
# MLP
MLP_HIDDEN_1 = 8  # Smaller for student
MLP_HIDDEN_2 = 4  # Smaller for student
# CNN
CNN_OUT_CHANNELS_1 = 4 # Smaller for student
CNN_KERNEL_SIZE_1 = 3
CNN_LINEAR_FEATURES = 8 # Smaller for student
# LSTM
LSTM_HIDDEN_SIZE = 16 # Smaller for student
LSTM_NUM_LAYERS = 1
# Attention
ATTN_EMBED_DIM = 16 # Smaller for student
ATTN_NUM_HEADS = 2  # Smaller for student
ATTN_LINEAR_FEATURES = 8 # Smaller for student
# Mamba
MAMBA_DIM = 16 # Smaller for student
MAMBA_LAYER = 1
...
class StockDataset(Dataset):
    def __init__(self, X, Y, days):
        if X.shape[0] != Y.shape[0]:
             raise ValueError(f"X and Y must have the same number of samples (X shape: {X.shape}, Y shape: {Y.shape})")
    ...
    def __getitem__(self, index):
        x_window = self.X[index : index + self.days, :] # Shape: (days, num_features)
...
class StockDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        X_train_raw = feature_data[:n_train]
        Y_train_raw = target_data[:n_train]
        X_val_raw = feature_data[n_train : n_train + n_val]
        Y_val_raw = target_data[n_train : n_train + n_val]
        X_test_raw = feature_data[n_train + n_val :]
        Y_test_raw = target_data[n_train + n_val :]
...
class StockForecastBase(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log('train_pred_loss', loss_pred, on_step=False, on_epoch=True, logger=True) # Log original loss too
    ...
    def on_test_epoch_end(self):
        ...
        if hasattr(self.trainer.datamodule, 'target_scaler'):
            ...
            print("Warning: Target scaler not found. Cannot calculate MSE on original scale.")
...
class StockMLP(StockForecastBase):
    ...
    def forward(self, x):
        if x.shape[1] == self.hparams.days_window: x = x.permute(0, 2, 1)
        x = self.flatten(x)
...
class StockAttention(StockForecastBase):
    def __init__(self, input_features, days_window, embed_dim=32, num_heads=4, linear_features=16, dropout_rate=0.2, learning_rate=0.001):
        super().__init__(learning_rate)
        self.save_hyperparameters()
        self.input_proj = nn.Linear(self.hparams.input_features, self.hparams.embed_dim) if self.hparams.input_features != self.hparams.embed_dim else nn.Identity()
        self.attention = nn.MultiheadAttention(embed_dim=self.hparams.embed_dim, num_heads=self.hparams.num_heads,
                                               dropout=self.hparams.dropout_rate, batch_first=False)
        self.flatten = nn.Flatten()
        self.fc1_in_features = self.hparams.days_window * self.hparams.embed_dim
        self.fc1 = nn.Linear(self.fc1_in_features, self.hparams.linear_features)
        self.relu1 = nn.ReLU()
        self.dropout = nn.Dropout(self.hparams.dropout_rate)
        self.fc2 = nn.Linear(self.hparams.linear_features, 1)
...
class StockMamba(StockForecastBase):
    def __init__(self, input_features, days_window, d_model=32, mamba_layer=1, dropout_rate=0.2, learning_rate=0.001):
        ...
        self.save_hyperparameters()
        self.input_proj = nn.Linear(self.hparams.input_features, self.hparams.d_model) if self.hparams.input_features != self.hparams.d_model else nn.Identity()
...
if __name__ == '__main__':
    ...
    # The hyperparameters in the config section now define the student
    if MODEL_TYPE == 'MLP':
        model = StockMLP(
            input_features=num_features, days_window=DAYS_WINDOW,
            hidden_size_1=MLP_HIDDEN_1, hidden_size_2=MLP_HIDDEN_2,
            dropout_rate=DROPOUT_RATE, learning_rate=LEARNING_RATE)
    elif MODEL_TYPE == 'CNN':
         if DAYS_WINDOW >= CNN_KERNEL_SIZE_1:
            model = StockCNN(
                input_features=num_features, days_window=DAYS_WINDOW,
                out_channels_1=CNN_OUT_CHANNELS_1, kernel_size_1=CNN_KERNEL_SIZE_1,
                linear_features=CNN_LINEAR_FEATURES, dropout_rate=DROPOUT_RATE,
                learning_rate=LEARNING_RATE)
         else: raise ValueError("DAYS_WINDOW must be >= CNN_KERNEL_SIZE_1")
    elif MODEL_TYPE == 'LSTM':
        model = StockLSTM(
            input_features=num_features, hidden_size=LSTM_HIDDEN_SIZE,
            num_layers=LSTM_NUM_LAYERS, dropout_rate=DROPOUT_RATE,
            learning_rate=LEARNING_RATE)
    elif MODEL_TYPE == 'Attention':
        model = StockAttention(
            input_features=num_features, days_window=DAYS_WINDOW,
            embed_dim=ATTN_EMBED_DIM, num_heads=ATTN_NUM_HEADS,
            linear_features=ATTN_LINEAR_FEATURES, dropout_rate=DROPOUT_RATE,
            learning_rate=LEARNING_RATE)
    elif MODEL_TYPE == 'Mamba':
        model = StockMamba(
            input_features=num_features, days_window=DAYS_WINDOW,
            d_model=MAMBA_DIM, mamba_layer=MAMBA_LAYER,
            dropout_rate=DROPOUT_RATE, learning_rate=LEARNING_RATE)
    else:
    ...
    logger = CSVLogger("logs", name=f"stock_forecast_{MODEL_TYPE}{'_distilled' if ENABLE_DISTILLATION else ''}")
